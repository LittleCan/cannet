...
namespace cannet
{
        ...
        if (state_ == kConnected)
        {
            ...
            else
            {
                loop_->runInLoop([this, buf]
                                 { sendInLoop(buf->retrieveAllAsString(), buf->readableBytes()); });
            }
        }
    }
    ...
    void TcpConnection::sendInLoop(const std::string &data, size_t len)
    {
        ...
        {
            nwrote = socketops::write(channel_->fd(), data.c_str(), len); //可以直接write
            if (nwrote >= 0)
            {
    ...
    void TcpConnection::shutdown()
    {
        ...
        if (state_ == kConnected)
        {
            ...
            loop_->runInLoop([capture0 = shared_from_this()]
                             { capture0->shutdownInLoop(); });
        }
    }

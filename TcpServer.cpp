...
namespace cannet
{
    ...
    void TcpServer::acceptHandRead()
    {
        ...
        while (true)
        {
            ...
            if (connFd >= 0) //得到了一个连接
            {
                LOG << "accept success" << inet_ntoa(peerAddr.sin_addr);
                if (true) // to_do: config
    ...
    void TcpServer::newConnection(int sockFd)
    {
        ...
        ioLoop->queueInLoop([ioLoop, sockFd, this]
                            { ioLoop->createConnection(sockFd, connectionCallback_, messageCallback_, writeCompleteCallback_); });
    }
}
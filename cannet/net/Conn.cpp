...
//        trace("tcp constructed %s - %s fd: %d", local_.toString().c_str(), peer_.toString().c_str(), fd);
    ...
    void
    TcpConn::connect(EventBase *base, const std::string &host, unsigned short port, int timeout,
...
//        trace("tcp closing %s - %s fd %d %d", local_.toString().c_str(), peer_.toString().c_str(),
...
//        fatalif(state_ != Handshaking, "handleHandshaking called when state_=%d", state_);
        ...
        if (r == 1 && pfd.revents == POLLOUT) {
            ...
            if (state_ == State::Connected) {
                connectedTime_ = Util::timeMilli();
//                trace("tcp connected %s - %s fd %d", local_.toString().c_str(), peer_.toString().c_str(),
//                      channel_->fd());
                if (statecb_) {
                    statecb_(con);
                }
...
//                error("write error: channel %lld fd %d wd %ld %d %s", (long long) channel_->id(), channel_->fd(), wd,
...
//            warn("connection %s - %s closed, but still writing %lu bytes", local_.toString().c_str(),
//                 peer_.toString().c_str(), buf.size());
        }
    }
...
//                 peer_.toString().c_str(), len);
    ...
    void TcpConn::onMsg(CodecBase *codec, const MsgCallBack &cb) {
        ...
        onRead([cb](const TcpConnPtr &con) {
            ...
            while (r) {
                Slice msg;
                r = con->codec_->tryDecode(con->getInput().data(), msg);
                if (r < 0) {
                    con->channel_->close();
                    break;
                } else if (r > 0) {
//                    trace("a msg decoded. origin len %d msg len %ld", r, msg.size());
    ...
    TcpServer::TcpServer(EventBases *bases) : base_(bases->allocBase()), bases_(bases), listen_channel_(NULL),

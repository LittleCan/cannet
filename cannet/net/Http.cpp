            ...
            *line1 = req.eatLine();
            while (req.size()) {
                ...
                if (k.size() && ln.size() && k.back() == ':') {
                    ...
                    headers[std::string(k.sub(0, -1))] = ln.data();
...
//                    error("bad http line: %.*s %.*s", (int) k.size(), k.data(), (int) ln.size(), ln.data());
    ...
    HttpMsg::Result HttpRequest::tryDecode(Slice buf, bool copyBody) {
        ...
        if (ln1.size()) {
            method = ln1.eatWord().data();
            query_uri = ln1.eatWord().data();
            version = ln1.eatWord().data();
...
//                error("query uri '%.*s' should begin with /", (int) query_uri.size(), query_uri.data());
            ...
            for (size_t i = 0; i < query_uri.size(); i++) {
                if (query_uri[i] == '?') {
                    uri = Slice(query_uri.data(), i).data();
                    ...
                    while (c < qs.size()) {
                        ...
                        if (c < qs.size() && qs[c] == '&')
                            c++;
                        if (kb != ke) {
                            args[std::string(qs.data() + kb, qs.data() + ke)] = std::string(qs.data() + vb,
                                                                                            qs.data() + ve);
                        }
                        ve = vb = ke = kb = c;
                    }
    ...
    void HttpConnPtr::handleRead(const HttpCallBack &cb) const {
        if (!tcp->isClient()) {  // server
            ...
            HttpMsg::Result r = req.tryDecode(tcp->getInput().data());
            if (r == HttpMsg::Error) {
                tcp->close();
                return;
            }
...
//                trace("http request:\n%.*s", (int) tcp->input_.size(), tcp->input_.data());
            ...
            HttpResponse &resp = getResponse();
            HttpMsg::Result r = resp.tryDecode(tcp->getInput().data());
            if (r == HttpMsg::Error) {
                tcp->close();
                return;
            }

...
namespace cannet {
    ...
    Ip4Addr::Ip4Addr(const std::string &host, unsigned short port) {
        ...
        if (addr_.sin_addr.s_addr == INADDR_NONE) {
            std::cout << "cannot resove %s to ip", host.c_str();
        }
    }

    std::string Ip4Addr::toString() const {
        uint32_t uip = addr_.sin_addr.s_addr;
        return Util::format("%d.%d.%d.%d:%d", (uip >> 0) & 0xff, (uip >> 8) & 0xff, (uip >> 16) & 0xff,
                            (uip >> 24) & 0xff, ntohs(addr_.sin_port));
    }

    std::string Ip4Addr::ip() const {
        uint32_t uip = addr_.sin_addr.s_addr;
        return Util::format("%d.%d.%d.%d", (uip >> 0) & 0xff, (uip >> 8) & 0xff, (uip >> 16) & 0xff,
                            (uip >> 24) & 0xff);
    }
